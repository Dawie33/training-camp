...
export function ProgressChart({ sessions }: ProgressChartProps) {
  ...
  const cellColor = (n: number) => {
    if (n === 0) return 'bg-border/60 border-border'
    if (n === 1) return 'bg-orange-600/25 border-orange-600/30'
    if (n === 2) return 'bg-orange-600/60 border-orange-600/70'
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-medium text-muted-foreground">Consistance des entraînements (12 semaines)</h3>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>Moins</span>
            {(['bg-border/60', 'bg-orange-600/25', 'bg-orange-600/60', 'bg-orange-600'] as const).map((c, i) => (
        ...
        <div className="flex gap-1.5 overflow-x-auto">
          ...
          <div className="flex flex-col gap-1 mr-1 shrink-0">
            {['Lun', '', 'Mer', '', 'Ven', '', 'Dim'].map((label, i) => (
              <div key={i} className="h-4 flex items-center text-xs text-muted-foreground w-7">
                {label}
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div>
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Sessions par jour de semaine</h3>
        ...
        {/* Volume hebdomadaire */}
        <div>
          <h3 className="text-sm font-medium text-muted-foreground mb-3">Volume par semaine (minutes)</h3>
          <div className="flex items-end gap-1.5 h-28">
            {weeklyVolume.map((week, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative h-full">
                <div className="w-full flex-1 flex items-end">
                  <motion.div
                    className="w-full bg-orange-600 rounded-t-sm hover:opacity-80 cursor-pointer"
                    initial={{ height: 0 }}
                ...
                {week.minutes > 0 && (
                  <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                    <div className="bg-card border border-border rounded-lg px-2 py-1.5 text-xs whitespace-nowrap">
                      <p className="font-semibold text-foreground">{week.minutes} min</p>
                      <p className="text-muted-foreground">{week.count} session{week.count > 1 ? 's' : ''}</p>
                    </div>
                  </div>
                )}
                <div className="text-center leading-tight text-[8px] text-muted-foreground">
