...
const selectClass = 'w-full px-3 py-2.5 rounded-md bg-secondary/40 border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-colors'
...
export function AiGenerateForm({
  ...
  return (
    <div className="bg-card rounded-lg p-4 lg:p-6 border border-border space-y-4 lg:space-y-5">
      <h2 className="text-lg lg:text-xl font-bold text-foreground">Paramètres du Workout</h2>
      ...
      <div className="flex items-center justify-between p-3 rounded-md bg-secondary/40 border border-border">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Adapte le workout à votre profil, 1RMs et historique</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Type de Workout</label>
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">
            Difficulté
            {personalized && <span className="ml-2 text-xs text-muted-foreground font-normal">(déterminé par votre profil)</span>}
          </label>
          <select
            value={difficulty}
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">
          Durée totale : <span className="text-primary font-bold">{duration} min</span>
        </label>
        <input
          type="range"
          min="15"
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-3">Environnement d&apos;entraînement</label>
          ...
          {MODES.map((m) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 p-3 rounded-md border text-center transition-all ${isActive
        ...
        {mode === 'bodyweight' && (
          <p className="text-xs text-muted-foreground">Workout sans matériel — poids du corps uniquement.</p>
        )}
        ...
        {mode === 'home' && profileEquipment.length > 0 && (
          <p className="text-xs text-muted-foreground">{profileEquipment.length} équipement{profileEquipment.length > 1 ? 's' : ''} depuis ton profil.</p>
        )}

        {mode === 'box' && (
          <p className="text-xs text-muted-foreground">Matériel complet d&apos;une box CrossFit — barre, rack, haltères, rameur, assault bike...</p>
        )}

        {mode === 'custom' && (
          <div className="space-y-3 rounded-md p-3 bg-secondary/40 border border-border">
            {EQUIPMENT_CATEGORIES.map((group) => (
              <div key={group.category}>
                <p className="text-xs font-semibold text-muted-foreground mb-1.5 uppercase tracking-wider">{group.category}</p>
            ...
            {equipment.length > 0 && (
              <p className="text-xs text-primary pt-1 font-medium">
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-2">Instructions additionnelles</label>
        <textarea
          value={additionalInstructions}
          onChange={(e) => setAdditionalInstructions(e.target.value)}
          placeholder="Ex: Focus haut du corps, sans sauts..."
          className="w-full px-3 py-2.5 rounded-md bg-secondary/40 border border-border text-foreground placeholder:text-muted-foreground min-h-[100px] focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-colors resize-none"
        />
      </div>
      ...
      <button
        onClick={onGenerate}
        disabled={loading}
        className="w-full px-6 py-3 lg:py-4 bg-primary hover:bg-primary/90 text-primary-foreground rounded-md font-semibold transition-all shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
      >
        {loading ? (
      ...
      {error && (
        <div className="p-4 bg-destructive/10 border border-destructive/30 rounded-md text-destructive">{error}</div>
      )}
    </div>
  )
}
