...
import { BookOpen, Clipboard, PenLine, Sparkles, Timer, Trophy } from 'lucide-react'
...
const IMPORT_ACTIONS = [
  {
    label: 'Coller un entraînement',
    description: 'Coller depuis un texte ou programme',
    icon: Clipboard,
...
export default function HyroxPage() {
  const { sessions, loading, handleDelete } = useHyroxDashboard()
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        <motion.div variants={fadeInUp} className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">8× (1km run + 1 station) — Préparation et suivi des temps</p>
          </div>
        </motion.div>
        ...
        <motion.div variants={fadeInUp}>
          ...
          {loading ? (
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
              ...
              <Link
                href="/hyrox/generate"
                className="inline-flex px-4 py-2 bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 rounded-lg hover:bg-yellow-500/30 transition-colors text-sm"
              >
                Générer avec l&apos;IA
              </Link>
            </div>
                ...
                return (
                  <div key={session.id} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl px-4 py-3 hover:bg-white/8 transition-colors">
                    ...
                    <div className="text-right flex-shrink-0">
                      {totalTime
                        ? <p className="text-sm font-mono font-bold text-yellow-400">{totalTime}</p>
      ...
      {pasteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl shadow-2xl p-5 space-y-4">
            ...
            <p className="text-xs text-slate-400">Colle ton programme HYROX — l&apos;IA va l&apos;analyser et créer ta séance.</p>
            <textarea
              rows={6}
              placeholder="Ex: 1km run + SkiErg 1000m + 1km run + Sled Push 50m..."
              value={pasteText}
              onChange={e => setPasteText(e.target.value)}
              className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-yellow-500 placeholder:text-slate-600 resize-none"
            />
