...
export default function AthxLibraryPage() {
  ...
  const filtered = useMemo(() => sessions.filter(s => {
    ...
    if (search) {
      ...
      const name = (s.ai_plan?.name ?? ATHX_SESSION_TYPE_LABELS[s.session_type]).toLowerCase()
      if (!name.includes(q)) return false
    }
    return true
  }), [sessions, typeFilter, search])

  const columns = useMemo<ColumnDef<AthxSession>[]>(() => [
    {
      id: 'name',
      ...
      accessorFn: row => row.ai_plan?.name ?? ATHX_SESSION_TYPE_LABELS[row.session_type],
    ...
    },
    {
      ...
      header: 'Type',
      cell: ({ row }) => <span className="px-2 py-0.5 rounded text-xs font-medium bg-purple-500/15 text-purple-400">{ATHX_SESSION_TYPE_LABELS[row.getValue('session_type') as AthxSessionType]}</span>,
    },
    {
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500"
            />
          </div>
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={() => setTypeFilter('')} className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${typeFilter === '' ? 'bg-purple-500/20 border-purple-500/40 text-purple-400' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white'}`}>Tous</button>
            {SESSION_TYPES.map(type => (
              <button key={type} onClick={() => setTypeFilter(type)} className={`px-2.5 py-1 rounded-lg text-xs font-medium border transition-all ${typeFilter === type ? 'bg-purple-500/20 border-purple-500/40 text-purple-400' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white'}`}>
            ...
            {loading ? (
              ...
              <TableRow className="border-slate-700/50">
                <TableCell colSpan={columns.length} className="h-24 text-center text-slate-500 text-sm">
                  {sessions.length === 0 ? 'Aucune séance ATHX pour l\'instant' : 'Aucun résultat'}
                </TableCell>
              </TableRow>
            ) : (
